...
def get_hf_personal_details(api_key, text):
    """Extract personal details (name, email, LinkedIn) using Hugging Face API."""
    headers = {"Authorization": f"Bearer {api_key}"}

    prompt = f"""Extract the following details from the resume in JSON format:
    - Name
    - Email
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return eval(result)  # Convert stringified JSON into a Python dictionary
        return {"name": "Error", "email": "Error", "linkedin": "Error"}
    except Exception as e:
        return {"name": "Error", "email": "Error", "linkedin": "Error", "error": str(e)}
...
def get_hf_analysis(api_key, text, job_desc=None):
    ...
    - Improvement Suggestions: 3 actionable suggestions
...
def full_analysis_pipeline(resume_text, hf_api_key=None, job_desc=None):
    ...
    # Personal Details
    if hf_api_key:
        personal_details = get_hf_personal_details(hf_api_key, resume_text)
        results['personal'] = {
            'name': personal_details.get('name', 'Not Found'),
            'email': personal_details.get('email', 'Not Found'),
            'linkedin': personal_details.get('linkedin', 'Not Found')
        }
    else:
        results['personal'] = {'name': 'API Key Missing', 'email': 'API Key Missing', 'linkedin': 'API Key Missing'}
